...
def validate_a2ui_schemas():
    ...
    approved_components = {
        ...
        "header", "select", "actions", "list", "action_card", "textarea", "buttons",
        "greeting", "number"
    }
    ...
    for fname in os.listdir(schema_dir):
        ...
        try:
            ...
            def check_components(o, fn=fname):
                if isinstance(o, dict):
                    if 'type' in o and o['type'] not in approved_components:
                        errors.append(f"{fn}: Unapproved component type '{o['type']}'")
                    # Check for inline JS or arbitrary script injections
                    for k, v in o.items():
                        if isinstance(v, str):
                            if 'javascript:' in v.lower() or '<script' in v.lower():
                                errors.append(f"{fn}: Script injection detected in property '{k}': {v}")
                    for v in o.values():
                        check_components(v, fn)
                elif isinstance(o, list):
                    for item in o:
                        check_components(item, fn)
