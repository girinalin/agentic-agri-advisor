...
def validate_safety():
    ...
    try:
        with open("agents/coordinator/agent.py", encoding="utf-8") as f:
            agent_code = f.read()

        assert "escalate" in agent_code or "safety" in agent_code.lower(), (
            "Safety Kernel escalation path missing from coordinator instructions"
        )
