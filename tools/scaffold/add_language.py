...
def check_translations_js(code: str) -> list[CheckResult]:
    ...
    if not path.exists():
        return [
            CheckResult(
                rel_path, "—", "FILE_NOT_FOUND", f"File does not exist: {rel_path}"
            )
        ]
...
def check_python_file(
    rel_path: str, code: str, sections: list[dict]
) -> list[CheckResult]:
            ...
            [
                {
                    "name": "TRANSLATIONS",
                    "anchor": f"'{code}':",
                    "description": "Main UI string dictionary",
                },
                {
                    "name": "SCHEMA_TRANSLATIONS",
                    "anchor": f"'{code}':",
                    "description": "A2UI schema translations (second occurrence)",
                },
                {
                    "name": "LANGUAGE_CONFIGS",
                    "anchor": f"'{code}':",
                    "description": "Locale config entry",
                },
            ],
        ),
        (
            ...
            [
                {
                    ...
                    "anchor": f'value="{code}"',
                    "description": "Language dropdown option",
                },
            ],
        ),
        (
            ...
            [
                {
                    ...
                    "anchor": f'"{code}"',
                    "description": "Alias map entry",
                },
                {
                    "name": "langNameMap",
                    "anchor": f'"{code}"',
                    "description": "Code → display name map",
                },
                {
                    "name": "langMap_bcp47",
                    "anchor": f'"{code}"',
                    "description": "BCP-47 map entries",
                },
                {
            ...
            [
                {
                    "name": "langMap_stt",
                    "anchor": f'"{code}"',
                    "description": "STT BCP-47 map",
                },
                {
                    "name": "codeToBcp47",
                    "anchor": f'"{code}"',
                    "description": "Code to BCP-47 map",
                },
            ],
        ),
        (
            ...
            [
                {
                    ...
                    "anchor": f'"{code}"',
                    "description": "Offline knowledge diagnostics",
                },
            ],
        ),
        (
            "ui/agui/local_models.js",
            "js",
            [
                {
                    "name": "labels",
                    "anchor": f'"{code}"',
                    "description": "Offline AI persona labels",
                },
            ],
        ),
        (
            ...
            [
                {
                    "name": "language-selector",
                    "anchor": f'value="{code}"',
                    "description": "Landing page language option",
                },
            ],
        ),
        (
            ...
            [
                {
                    "name": "TRANSLATIONS",
                    "anchor": f"TRANSLATIONS['{code}']",
                    "description": "Landing page translation block",
                },
            ],
        ),
        # ── Backend Python ─────────────────────────────────────────────────────
            ...
            [
                {
                    "name": "VOICE_MAP",
                    "anchor": f'"{code}"',
                    "description": "Edge-TTS voice mapping",
                },
            ],
        ),
        (
            "mcp_servers/translation/server.py",
            "python",
            [
                {
                    "name": "MOCK_TRANSLATIONS",
                    "anchor": f'"{code}"',
                    "description": "Translation mock entries",
                },
            ],
        ),
        # ── Agent instructions ─────────────────────────────────────────────────
            ...
            [
                {
                    ...
                    "anchor": f"'{code}'",
                    "description": "Language parameter examples",
                },
            ],
        ),
        (
            ...
            [
                {
                    "name": "LANGUAGE_RULE",
                    "anchor": f"Respond in {code}",
                    "description": "Language rule entry",
                },
            ],
        ),
        (
...
def _check_file(
    rel_path: str, file_type: str, code: str, sections: list[dict]
) -> list[CheckResult]:
...
def print_report(results: list[CheckResult], code: str, name: str) -> int:
    ...
    if missing or not_found:
        ...
        print()
        print("── Next Steps ────────────────────────────────────────────")
        print("  Read: .context/04-add-language.md")
        print("  Guide: .context/change-maps/add-language.yaml")
        print(
            f"  Then re-run: python tools/scaffold/add_language.py --code {code} --name {name} --check"
        )
        return 1
    ...
    print("  node -c ui/agui/translations.js && node -c ui/agui/dashboard.js")
    print("  make test")
    return 0


def write_evidence(
    code: str, name: str, results: list[CheckResult], exit_code: int
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--code", required=True, help="2-letter language code (e.g. kn)"
    )
    parser.add_argument(
        "--name", required=True, help="English language name (e.g. Kannada)"
    )
    parser.add_argument(
        "--native",
        default="",
        help="Native script name (e.g. ಕನ್ನಡ) — for documentation",
    )
    parser.add_argument(
        "--check",
        action="store_true",
        default=True,
        help="Run audit and report (default)",
    )
    parser.add_argument(
        "--evidence",
        action="store_true",
        default=False,
        help="Write evidence artifact to .ai-sdlc/evidence/",
    )
    ...
    if len(code) != 2:
        print(
            f"ERROR: --code must be a 2-letter ISO 639-1 code, got: {code!r}",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    print(
        f"Auditing language: {args.name} ({code})"
        + (f" / {args.native}" if args.native else "")
    )
