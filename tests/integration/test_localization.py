...
def test_translation_keys_defined():
    ...
    for lang in languages:
        ...
        assert lang in schema_translations, f"Language '{lang}' missing in SCHEMA_TRANSLATIONS"
...
def test_script_separation_and_leak_prevention():
    ...
    for key, val in translations['hi'].items():
        assert not telugu_regex.search(val), f"Telugu character leak in Hindi TRANSLATIONS for key '{key}': {val}"
    for key, val in schema_translations['hi'].items():
        assert not telugu_regex.search(val), f"Telugu character leak in Hindi SCHEMA_TRANSLATIONS for key '{key}': {val}"

    # 2. Check Telugu mode contains no Devanagari characters
    for key, val in translations['te'].items():
        assert not devanagari_regex.search(val), f"Devanagari character leak in Telugu TRANSLATIONS for key '{key}': {val}"
    for key, val in schema_translations['te'].items():
        assert not devanagari_regex.search(val), f"Devanagari character leak in Telugu SCHEMA_TRANSLATIONS for key '{key}': {val}"
...
def test_english_translation_cleanliness():
    ...
    for key, val in translations['en'].items():
        assert not telugu_regex.search(val), f"Leak in English TRANSLATIONS for key '{key}'"
        assert not devanagari_regex.search(val), f"Leak in English TRANSLATIONS for key '{key}'"

    for key, val in schema_translations['en'].items():
        assert not telugu_regex.search(val), f"Leak in English SCHEMA_TRANSLATIONS for key '{key}'"
        is_raw_key = '.' in val and ' ' not in val and not val.endswith('.')
        assert not is_raw_key, f"Suspected raw dot key in English value for key '{key}': {val}"
