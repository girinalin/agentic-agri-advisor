...
def test_landing_page():
    with sync_playwright() as p:
        ...
        guest_modal = page.query_selector('#guest-modal') or ''
        ...
        guest_login = page.query_selector('#guest-enter-btn, #enter-as-guest-btn') or None
        if guest_login:
...
def test_guest_login_flow():
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=False)  # headless=True for testing in CI, False to see
        page = browser.new_page(viewport={"width": 375, "height": 812})
...
def test_simple_query_flow():
    with sync_playwright() as p:
        ...
        for selector in ['#user-input-field', '#advisor-query', '[data-tr-placeholder="chat_placeholder"]']:
        ...
        for btn_selector in ['#send-btn', '#advisor-send', '[data-tr="btn_send"]', 'button:has-text("Send")']:
        ...
        try:
            ...
            page.wait_for_selector('.message:has-text("Sastri"), .expert-msg, #sastri-response', timeout=5000)
...
def test_expert_delegation():
    ...
    with sync_playwright() as p:
        ...
        advisor_options = page.query_selector_all('#advisor-btn, #expert-btn, .escalation-link')
        ...
        for btn in page.query_selector_all('#send-btn, #advisor-send, button:has-text("Send")'):
        ...
        try:
            ...
            page.wait_for_selector('.message:has-text("Sastri"), .expert-msg, #sastri-response', timeout=15000)
            print("✅ Response received!")
        except Exception:
            # Maybe the response is in the Sastri chat or expert form showed up  
            try:
                # Try waiting for either: simple response OR escalation UI
                page.wait_for_selector('.expert-form-container, .escalation-ui', timeout=5000)
