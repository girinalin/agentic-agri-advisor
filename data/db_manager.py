...
def log_activity_record(planting_id, activity_type, quantity, unit, details, timestamp=None):
    """
    ...
    try:
        ...
        cursor.execute(
            ...
            (activity_id, planting_id, activity_type, float(quantity), unit, details, timestamp)
        )
        conn.commit()
...
def get_daily_plans(planting_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM farm_plans WHERE planting_id = ? ORDER BY timestamp DESC", (planting_id,))
...
def update_plan_state(plan_id, state):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE farm_plans SET state = ? WHERE plan_id = ?", (state, plan_id))
        conn.commit()
        return True
    finally:
        conn.close()
...
def get_reminders(planting_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM reminders WHERE planting_id = ? ORDER BY timestamp DESC", (planting_id,))
        return [dict(row) for row in cursor.fetchall()]
    finally:
        conn.close()
...
def update_reminder_state(reminder_id, state):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE reminders SET state = ? WHERE reminder_id = ?", (state, reminder_id))
        conn.commit()
        return True
    finally:
        conn.close()
...
def get_escalations(planting_id):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("SELECT * FROM escalations WHERE planting_id = ? ORDER BY timestamp DESC", (planting_id,))
        return [dict(row) for row in cursor.fetchall()]
    finally:
        conn.close()
...
def update_expert_case_state(escalation_id: str, state: str, expert_response: str = None) -> dict:
...
def rollback_governance_version(content_id: str) -> dict:
    ...
    try:
        ...
        cursor.execute("SELECT version FROM okf_governance WHERE content_id = ?", (content_id,))
        row = cursor.fetchone()
        if row:
            v = row[0]
            if v > 1:
...
def log_observability_event(correlation_id: str, event_type: str, screen: str = "", agent: str = "", tool: str = "", route: str = "", safety_decision: str = "", latency: float = 0.0, device_tier: str = "") -> dict:
    ...
    try:
        ...
        cursor.execute(
            ...
            (correlation_id, event_type, screen, agent, tool, route, safety_decision, latency, device_tier)
        )
        conn.commit()
        return {"status": "success"}
    finally:
...
def get_observability_logs() -> list:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT correlation_id, event_type, screen, route, latency, timestamp FROM observability_logs ORDER BY timestamp DESC LIMIT 50")
...
def delete_farm_data(user_id: str) -> dict:
    ...
    try:
        ...
        cursor.execute("SELECT planting_id FROM plantings WHERE field_id IN (SELECT field_id FROM fields WHERE farmer_id = ?)", (user_id,))
        ...
        cursor.execute("DELETE FROM plantings WHERE field_id IN (SELECT field_id FROM fields WHERE farmer_id = ?)", (user_id,))
...
def export_farm_data(user_id: str) -> dict:
    ...
    try:
        ...
        cursor.execute("SELECT * FROM privacy_preferences WHERE user_id = ?", (user_id,))
        data["privacy"] = [dict(r) for r in cursor.fetchall()]

        # Get plantings
        cursor.execute("SELECT * FROM plantings WHERE field_id IN (SELECT field_id FROM fields WHERE farmer_id = ?)", (user_id,))
        ...
        if planting_ids:
            ...
            cursor.execute(f"SELECT * FROM activities WHERE planting_id IN ({placeholders})", planting_ids)
            data["activities"] = [dict(r) for r in cursor.fetchall()]
            cursor.execute(f"SELECT * FROM farm_plans WHERE planting_id IN ({placeholders})", planting_ids)
            data["plans"] = [dict(r) for r in cursor.fetchall()]
            cursor.execute(f"SELECT * FROM reminders WHERE planting_id IN ({placeholders})", planting_ids)
            data["reminders"] = [dict(r) for r in cursor.fetchall()]
            cursor.execute(f"SELECT * FROM escalations WHERE planting_id IN ({placeholders})", planting_ids)
            data["escalations"] = [dict(r) for r in cursor.fetchall()]
            cursor.execute(f"SELECT * FROM feedbacks WHERE planting_id IN ({placeholders})", planting_ids)
